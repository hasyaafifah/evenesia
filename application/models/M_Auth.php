...
class M_Auth extends CI_Model
{
	...
	private function loginVendor($nip, $password)
	{
		$q = $this->db->select('*')->where(array('nip' => $nip, 'password' => md5($password)))->get('tb_vendor');
		return $q;
	}
	public function doLogin($nip, $password)
	{
		...
		$cek_login_vendor = $this->loginVendor($nip, $password);
		...
		} else if ($cek_login_vendor->num_rows()) {
			$d = $cek_login_vendor->row();
			$this->session->set_userdata('is_logged_in', 'login');
			$this->session->set_userdata('user_type', $d->type);
